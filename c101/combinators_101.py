...
def object_get(obj: Any) -> Callable[[str], Any]:
    return lambda name, *default: getattr(obj, name, *default)
...
def format_args(name, args, kwargs):
    return (
        f"{name}("
        + ", ".join(
            [repr(x) for x in args] + [f"{k!r}={v!r}" for k, v in kwargs.items()]
        )
        + ")"
    )
...
h = except_(
    plus_three, TypeError, compose(partial(logging.error, "plus_three: %s"), repr)
)
h(2)
h("Nope")
