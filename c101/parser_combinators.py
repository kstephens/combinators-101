...
def grammar_parser():
    ...
    definition(
        "string", lexeme(r'^"(\\"|\\?[^"]+)*"', lambda m: ("string", eval(m[0])))
    )
    definition(
        "regex",
        lexeme(r"^/((\\/|(\\?[^/]+))*)/", lambda m: ("regex", re.compile(m[1]))),
    )
    definition("name", lexeme(r"^[a-zA-Z][a-zA-Z0-9_]*"))
    definition("terminal", alternation(_("string"), _("regex")))
    definition("non_terminal", _("name"), lambda x: ("reference", x))
    definition("action", lexeme(r"^@\{(.+?)\}@", lambda m: ("action", m[1].strip())))
    definition("basic_match", alternation(_("non_terminal"), _("terminal")))
    definition(
        "bound_match",
        sequence_of(one(_("basic_match")), one(lexeme(r"^:")), one(_("name"))),
    ...
    definition(
        "alternation",
        sequence_of(one(_("sequence")), one(lexeme(r"\|")), one(_("production"))),
    ...
    definition(
        "definition",
        sequence_of(
            one(_("name")), one(lexeme(r"=")), one(_("production")), one(lexeme(r";"))
        ),
